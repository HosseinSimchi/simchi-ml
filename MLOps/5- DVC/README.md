...
- ###### `python -m dvc remote modify minio endpointurl https://minio.hosseinsimchi.com`
  - ###### `endpoint-url` or `endpointurl`
- ###### Setting username and password to access minIO
  - ###### `python -m dvc remote modify --local minio access_key_id HosseinSimchi`
...
- ###### اگر که داده ها تغییر میکنند باید اونو به کنترل ورژن داده ها اعلام کنیم
  - ###### `python -m dvc add ./data/cleaned_dataset.csv`
  - ###### `git add .`
